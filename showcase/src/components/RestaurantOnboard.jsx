...
const restaurantsData = [
  {
    name: 'Mr. Burger',
    description: 'Lorem ipsum dolor sit amet consectetur. Duis in duis faucibus cras ac quis aliquet dolor. Amet urna quam nunc sit velit faucibus id. Lectus facilisis volutpat neque amet non.',
    rating: 4,
    image: '/path_to_images/mr_burger.png',
  },
  {
    name: 'Swadu Cafe',
    description: 'Duis in duis faucibus cras ac quis aliquet dolor. Amet urna quam nunc sit velit faucibus id. Lectus facilisis volutpat neque amet non.',
    rating: 5,
...
const RestaurantOnboard = () => {
  ...
  return (
    <div className="p-8 md:p-12 lg:p-16 xl:p-24 max-w-screen-xl mx-auto">
      <h2 className="text-2xl md:text-5xl font-bold mb-6 text-center">RESTAURANTS ONBOARD</h2>
      
      {/* Restaurant Info */}
      <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
        <div className="mx-4 flex-1">
          <h3 className="text-2xl md:text-3xl font-semibold text-[#ff7a59] mb-4">{selectedRestaurant.name}</h3>
          <p className="text-gray-600 text-base md:text-lg mb-4">{selectedRestaurant.description}</p>
        ...
        <div className="mt-6 ml-6 relative flex-1">
          ...
          {/* Restaurant Image */}
          <img src={rest} alt={selectedRestaurant.name} className="relative z-20 w-full h-auto rounded-lg md:w-80 lg:w-96" />
        </div>
      </div>
        ...
        {restaurantsData.map((restaurant, index) => (
          <button
            ...
            className={`py-2 px-4 rounded-lg shadow-md ${
              selectedRestaurant.name === restaurant.name ? 'bg-[#00643c] text-white' : 'bg-white text-black'
            }`}
          >
            {restaurant.name}
          </button>
